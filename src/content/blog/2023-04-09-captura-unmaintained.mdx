...
related:
  ...
  - 2026-3-12-fate-grand-automata-developer-guide
---

import CapturaDonations from '@components/blog/CapturaDonations.astro';
import CapturaWebRecorderLink from '@components/blog/CapturaWebRecorderLink.astro';
import capturaImage from '@images/captura.png';

## What's Captura?
Captura **was** a screen capture (screenshot, recording) app for Windows which could capture screen, webcam, audio and keystrokes.
It is no longer maintained. The original open-source repository remains archived on GitHub (search for **Captura** screen recorder) with 8k+ stars and 1.5k+ forks.

<CapturaWebRecorderLink />

## Status / TL;DR
- **Maintained?** No. Development stopped around 2019.
- **Download:** Historical releases remain on the archived Captura GitHub repository (look under **Releases → Assets** for Portable or Setup packages). Prefer the [Captura Web Recorder](/tools/captura/) for new use.
...
<Picture src={capturaImage} alt="Captura" />
...
Removing that information is a violation of the license terms (documented in the upstream Captura issue tracker as **#405: Illegal Rebrand**).
...
- **Screen recording (browser):** [Captura Web Recorder](/tools/captura/) — a spiritual successor to Captura that runs entirely in Chrome with no installation required.
...
- I have since worked on [another open-source project](/blog/2026/03/12/fate-grand-automata-developer-guide.html) with a narrower scope and a different community dynamic — and found it far less draining. My current full-time role at a major tech company leaves limited time for open source, so any future contributions would more likely be to existing projects than to new ones I create.

## References
Upstream Captura issue **#405: Illegal Rebrand** (archived repository issue tracker).
