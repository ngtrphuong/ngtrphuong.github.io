...
description: "Pre-bake your entire build toolchain into a Docker image and cut CI build times dramatically. Teaches: custom Docker base images, GitHub Container Registry, GitHub Actions optimisation."
icon: "fab fa-docker"
...
jobs:
  build:
    runs-on: ubuntu-latest
    container:
      image: ghcr.io/ngtrphuong/ngtrphuong-github-io-ci:latest
    steps:
      - name: Checkout
...
- name: Generate image tags
  ...
  run: |
    ...
    echo "tags=ghcr.io/ngtrphuong/ngtrphuong-github-io-ci:latest,ghcr.io/ngtrphuong/ngtrphuong-github-io-ci:df-${DOCKERFILE_HASH}" >> "$GITHUB_OUTPUT"
```
...
- **Co-located with the code** — the image lives at `ghcr.io/ngtrphuong/...`, directly associated with the repository. Easy to find, easy to audit.
...
| GHCR | Stores the image at `ghcr.io/ngtrphuong/ngtrphuong-github-io-ci` |
...
*If you're also running a Jekyll site on GitHub Actions with custom plugins, the <PostLink slug="2026-04-01-pr-preview-setup" /> is worth reading alongside this one — together they cover the full CI pipeline this site uses.*
